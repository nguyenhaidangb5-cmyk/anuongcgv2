...
/**
 * ============================================================
 * CÔNG CỤ IMPORT CSV HÀNG LOẠT
 * Tạo/cập nhật quán ăn từ file CSV. Không đụng đến ảnh.
 * ============================================================
 */
add_action('admin_menu', function () {
    add_submenu_page(
        ...
        'manage_options',
        'cg-import-csv',
        'cg_render_csv_import_page'
    );
});

/**
 * Tạo file CSV template để user download
 */
add_action('admin_init', function () {
    if (
        isset($_GET['cg_action']) &&
        $_GET['cg_action'] === 'download_csv_template' &&
        current_user_can('manage_options') &&
        isset($_GET['_wpnonce']) &&
        wp_verify_nonce($_GET['_wpnonce'], 'cg_download_template')
    ) {
        $filename = 'quan_an_import_template.csv';
        ...
        // Header row
        fputcsv($output, [
            'ten_quan',          // Tên quán (bắt buộc)
            'mo_ta',             // Mô tả ngắn (excerpt)
            'sdt',               // Số điện thoại
            'dia_chi',           // Địa chỉ
            'gio_mo_cua',        // Giờ mở cửa: VD 07:00 - 22:00
            'khoang_gia',        // under-30k | 30k-50k | 50k-100k | over-100k
            'link_ggmap',        // Link Google Maps
            'so_zalo',           // Số Zalo
            'loai_hinh',         // Tên danh mục Loại hình: Ăn Sáng | Cơm | Hải Sản | Lẩu & Nướng | Đồ ăn vặt | Đặc sản địa phương | Món chay | Quán nhậu | Phở | Bún | Trà sữa/Cafe | Bánh mì
            'khu_vuc',           // Khu vực: Thị trấn Cần Giuộc | Xã Phước Lý | Xã Mỹ Lộc | Xã Phước Vĩnh Tây | Xã Tân Tập
            'may_lanh',          // 1 = có, 0 = không
            'cho_giu_xe',        // 1/0
            'da_xac_thuc',       // 1/0
            'dan_dia_phuong',    // 1/0
            'quan_moi',          // 1/0
            'dang_hot',          // 1/0
            'phu_hop_gia_dinh',  // 1/0
            'view_dep',          // 1/0
            'ngon_bo_re',        // 1/0
            'chuan_vi',          // 1/0
            'co_ruou_bia',       // 1/0
            'giao_hang',         // 1/0
            'diem_chat_luong',   // 0-10, bước 0.5
            'diem_gia_ca',       // 0-10
            'diem_phuc_vu',      // 0-10
            'diem_khong_gian',   // 0-10
        ]);

        // Dòng ví dụ mẫu
        fputcsv($output, [
            'Quán Cơm Bà Năm',
            'Quán cơm gia đình ngon bổ rẻ, nấu theo kiểu miền Nam truyền thống.',
            '0901234567',
            '123 QL50, Thị trấn Cần Giuộc',
            '06:00 - 20:00',
            'under-30k',
...
/**
 * Render trang Import CSV
 */
function cg_render_csv_import_page()
{
    if (!current_user_can('manage_options')) {
        wp_die('Bạn không có quyền truy cập trang này.');
    }
    ...
    // ── Xử lý form submit ──────────────────────────────────────────
    if (
        isset($_POST['cg_do_import_csv']) &&
        isset($_POST['cg_import_nonce']) &&
        wp_verify_nonce($_POST['cg_import_nonce'], 'cg_import_csv_action')
    ) {
        if (empty($_FILES['cg_csv_file']['tmp_name'])) {
            $error_global = 'Vui lòng chọn file CSV trước khi import.';
        } else {
            $results = cg_process_csv_import($_FILES['cg_csv_file']['tmp_name']);
        }
    }

    // URL download template
    $template_url = wp_nonce_url(
        admin_url('admin.php?cg_action=download_csv_template'),
        'cg_download_template'
    );
    ?>
    <div class="wrap">
        <h1>📥 Import Quán Ăn từ CSV</h1>
        <p style="color:#666; font-size:14px;">
            Tải file CSV template, điền data, rồi upload lại để tạo/cập nhật hàng loạt quán ăn.<br>
            <strong>Ảnh đã upload sẽ không bị ảnh hưởng.</strong>
        </p>

        <?php if ($error_global): ?>
            <div class="notice notice-error"><p><?php echo esc_html($error_global); ?></p></div>
        <?php endif; ?>

        <?php if ($results !== null): ?>
            <div class="notice notice-<?php echo ($results['errors'] > 0 && $results['success'] === 0) ? 'error' : 'success'; ?> is-dismissible">
                <p>
                    ✅ <strong>Tạo mới:</strong> <?php echo $results['created']; ?> quán &nbsp;|&nbsp;
                    🔄 <strong>Cập nhật:</strong> <?php echo $results['updated']; ?> quán &nbsp;|&nbsp;
                    ❌ <strong>Lỗi:</strong> <?php echo $results['errors']; ?> dòng
                </p>
            </div>
            <?php if (!empty($results['log'])): ?>
                <div style="background:#f9f9f9; border:1px solid #ddd; border-radius:4px; padding:15px; max-height:300px; overflow-y:auto; margin-bottom:20px; font-size:13px; font-family:monospace;">
                    <?php foreach ($results['log'] as $log_line): ?>
                        <div style="padding:2px 0; color:<?php echo strpos($log_line, '❌') !== false ? '#c00' : '#333'; ?>">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; max-width:900px;">
            ...
            <div style="background:#fff; border:1px solid #c3c4c7; border-radius:4px; padding:24px;">
                <h2 style="margin-top:0; font-size:16px;">📋 Hướng dẫn sử dụng</h2>
                <ol style="font-size:13px; line-height:1.8; padding-left:18px;">
                    <li>Download file <strong>CSV Template mẫu</strong> bên dưới</li>
                    <li>Mở bằng <strong>Google Sheets</strong> hoặc Excel (Encoding: UTF-8)</li>
                    <li>Điền data từ dòng thứ 3 trở xuống (dòng 1 = header, dòng 2 = ví dụ)</li>
                    <li>Export ra CSV (File → Download → CSV)</li>
                    <li>Upload file CSV vào form bên phải → nhấn Import</li>
                </ol>

                <a href="<?php echo esc_url($template_url); ?>" class="button button-secondary" style="margin-top:8px;">
                    ⬇️ Download CSV Template
                </a>

                <hr style="margin: 20px 0;">
                <h3 style="font-size:14px; margin:0 0 10px 0;">📌 Các giá trị hợp lệ</h3>
                <table style="font-size:12px; width:100%; border-collapse:collapse;">
                    <tr style="background:#f0f0f0;">
                        <th style="padding:6px 8px; text-align:left; border:1px solid #ddd;">Cột</th>
                        <th style="padding:6px 8px; text-align:left; border:1px solid #ddd;">Giá trị chấp nhận</th>
                    </tr>
                    <tr>
                        <td style="padding:5px 8px; border:1px solid #ddd;"><code>khoang_gia</code></td>
                        <td style="padding:5px 8px; border:1px solid #ddd;">under-30k · 30k-50k · 50k-100k · over-100k</td>
                    </tr>
                    <tr style="background:#f9f9f9;">
                        <td style="padding:5px 8px; border:1px solid #ddd;"><code>loai_hinh</code></td>
                        <td style="padding:5px 8px; border:1px solid #ddd;">Ăn Sáng · Cơm · Phở · Bún · Hải Sản · Lẩu &amp; Nướng · Đồ ăn vặt · Đặc sản địa phương · Món chay · Quán nhậu · Trà sữa/Cafe · Bánh mì</td>
                    </tr>
                    <tr>
                        <td style="padding:5px 8px; border:1px solid #ddd;"><code>khu_vuc</code></td>
                        <td style="padding:5px 8px; border:1px solid #ddd;">Thị trấn Cần Giuộc · Xã Phước Lý · Xã Mỹ Lộc · Xã Phước Vĩnh Tây · Xã Tân Tập</td>
                    </tr>
                    <tr style="background:#f9f9f9;">
                        <td style="padding:5px 8px; border:1px solid #ddd;">Checkbox (12 cột)</td>
                        <td style="padding:5px 8px; border:1px solid #ddd;"><strong>1</strong> = có &nbsp;·&nbsp; <strong>0</strong> hoặc để trống = không</td>
                    </tr>
                    <tr>
                        <td style="padding:5px 8px; border:1px solid #ddd;">Điểm đánh giá</td>
                        <td style="padding:5px 8px; border:1px solid #ddd;">Số từ 0 đến 10, bước 0.5 (VD: 8.5)</td>
                    </tr>
                </table>

                <div style="background:#e8f5e9; border:1px solid #4caf50; border-radius:4px; padding:10px 14px; margin-top:14px; font-size:12px;">
                    💡 <strong>Cập nhật quán đã có:</strong> Nếu tên quán (slug) đã tồn tại trong hệ thống, tool sẽ <strong>cập nhật</strong> thay vì tạo mới. Ảnh sẽ được giữ nguyên.
                </div>
            </div>

            <!-- Cột phải: Form upload -->
            <div style="background:#fff; border:1px solid #c3c4c7; border-radius:4px; padding:24px;">
                <h2 style="margin-top:0; font-size:16px;">📤 Upload & Import</h2>
                <form method="post" enctype="multipart/form-data">
                    <?php wp_nonce_field('cg_import_csv_action', 'cg_import_nonce'); ?>

                    <div style="border: 2px dashed #2271b1; border-radius:8px; padding:30px; text-align:center; background:#f0f6ff; margin-bottom:20px;">
                        <div style="font-size:48px; margin-bottom:10px;">📄</div>
                        <label for="cg_csv_file" style="display:block; font-weight:600; margin-bottom:10px; cursor:pointer;">
                            Chọn file CSV để upload
                        </label>
                        <input type="file" id="cg_csv_file" name="cg_csv_file" accept=".csv" style="max-width:280px;" required />
                        <p style="margin:10px 0 0; color:#666; font-size:12px;">Chỉ chấp nhận file .csv · Encoding: UTF-8</p>
                    </div>

                    <div style="background:#fff3cd; border:1px solid #ffc107; border-radius:4px; padding:10px 14px; margin-bottom:20px; font-size:13px;">
                        ⚠️ Kiểm tra kỹ data trước khi import. Tool sẽ tạo/cập nhật ngay lập tức.
                    </div>

                    <input
                        type="submit"
                        name="cg_do_import_csv"
                        class="button button-primary button-large"
                        value="📥 Bắt đầu Import"
                        style="width:100%;"
                        onclick="return confirm('Bắt đầu import CSV? Thao tác này sẽ tạo/cập nhật các quán ngay.');"
                    />
                </form>
            </div>
        </div>
...
/**
 * Hàm xử lý core: đọc CSV và tạo/cập nhật posts
 */
function cg_process_csv_import($tmp_file_path)
{
    $log     = [];
    $created = 0;
    $updated = 0;
    ...
    if (!$handle) {
        return ['created' => 0, 'updated' => 0, 'errors' => 1, 'log' => ['❌ Không thể đọc file CSV.']];
    }
    ...
    if (!$headers) {
        fclose($handle);
        return ['created' => 0, 'updated' => 0, 'errors' => 1, 'log' => ['❌ File CSV trống hoặc lỗi format.']];
    }
    ...
    while (($row = fgetcsv($handle)) !== false) {
        ...
        // Helper lấy giá trị theo tên cột
        $get = function ($key) use ($row, $col) {
            if (!isset($col[$key])) return '';
            return isset($row[$col[$key]]) ? trim($row[$col[$key]]) : '';
        };

        $ten_quan = $get('ten_quan');
        if (empty($ten_quan)) {
            $log[] = "❌ Dòng {$row_num}: Thiếu tên quán, bỏ qua.";
            $errors++;
            continue;
        }

        // Tìm post đã tồn tại theo title (so sánh slug)
        $slug          = sanitize_title($ten_quan);
        $existing_post = get_page_by_path($slug, OBJECT, 'quan_an');
        $post_id       = null;

        if ($existing_post) {
            // Cập nhật post hiện có
            wp_update_post(['ID' => $existing_post->ID, 'post_title' => $ten_quan, 'post_excerpt' => $get('mo_ta')]);
            ...
            $post_id = wp_insert_post([
                'post_title'   => $ten_quan,
                'post_excerpt' => $get('mo_ta'),
                'post_status'  => 'publish',
                'post_type'    => 'quan_an',
                'post_name'    => $slug,
            ]);
            if (is_wp_error($post_id)) {
                $log[] = "❌ Dòng {$row_num} ({$ten_quan}): " . $post_id->get_error_message();
                $errors++;
                continue;
            }
            $created++;
        ...
        // ── Cập nhật các meta field ────────────────────────────────

        // Thông tin cơ bản
        $basic_fields = [
            '_cg_phone'       => 'sdt',
            '_cg_address'     => 'dia_chi',
            '_cg_hours'       => 'gio_mo_cua',
            '_cg_price_range' => 'khoang_gia',
            '_cg_map_link'    => 'link_ggmap',
            '_cg_zalo_phone'  => 'so_zalo',
        ];
        foreach ($basic_fields as $meta_key => $csv_col) {
            $val = $get($csv_col);
            if ($val !== '') {
        ...
        $checkbox_fields = [
            '_cg_has_ac'           => 'may_lanh',
            '_cg_has_parking'      => 'cho_giu_xe',
            '_cg_is_verified'      => 'da_xac_thuc',
            '_cg_is_local_choice'  => 'dan_dia_phuong',
            '_cg_is_new'           => 'quan_moi',
            '_cg_is_trending'      => 'dang_hot',
            '_cg_is_family_friendly' => 'phu_hop_gia_dinh',
            '_cg_has_nice_view'    => 'view_dep',
            '_cg_is_good_cheap'    => 'ngon_bo_re',
            '_cg_is_authentic'     => 'chuan_vi',
            '_cg_has_alcohol'      => 'co_ruou_bia',
            '_cg_is_shipping'      => 'giao_hang',
        ];
        foreach ($checkbox_fields as $meta_key => $csv_col) {
            $val = $get($csv_col);
            // Chỉ update nếu cột có trong CSV
            if (isset($col[$csv_col])) {
                update_post_meta($post_id, $meta_key, ($val === '1') ? '1' : '');
            }
        }

        // Ratings
        $rating_food     = (float) str_replace(',', '.', $get('diem_chat_luong'));
        $rating_price    = (float) str_replace(',', '.', $get('diem_gia_ca'));
        $rating_service  = (float) str_replace(',', '.', $get('diem_phuc_vu'));
        $rating_ambiance = (float) str_replace(',', '.', $get('diem_khong_gian'));

        if (isset($col['diem_chat_luong'])) update_post_meta($post_id, '_cg_rating_food',     $rating_food);
        if (isset($col['diem_gia_ca']))     update_post_meta($post_id, '_cg_rating_price',    $rating_price);
        if (isset($col['diem_phuc_vu']))    update_post_meta($post_id, '_cg_rating_service',  $rating_service);
        if (isset($col['diem_khong_gian'])) update_post_meta($post_id, '_cg_rating_ambiance', $rating_ambiance);

        // Tính average_rating
        $vals = array_filter([$rating_food, $rating_price, $rating_service, $rating_ambiance], fn($v) => $v > 0);
        ...
        // Loại hình (food_type)
        $loai_hinh = $get('loai_hinh');
        if (!empty($loai_hinh)) {
            $term = get_term_by('name', $loai_hinh, 'food_type');
            if (!$term) {
                // Tự tạo term nếu chưa có
                $new_term = wp_insert_term($loai_hinh, 'food_type');
                $term_id  = is_wp_error($new_term) ? null : $new_term['term_id'];
            } else {
                $term_id = $term->term_id;
            }
            if ($term_id) {
                wp_set_post_terms($post_id, [$term_id], 'food_type');
            }
        }

        // Khu vực (khu_vuc)
        $khu_vuc_val = $get('khu_vuc');
        if (!empty($khu_vuc_val)) {
            $term = get_term_by('name', $khu_vuc_val, 'khu_vuc');
            if (!$term) {
                $new_term = wp_insert_term($khu_vuc_val, 'khu_vuc');
                $term_id  = is_wp_error($new_term) ? null : $new_term['term_id'];
            } else {
                $term_id = $term->term_id;
            }
            if ($term_id) {
                wp_set_post_terms($post_id, [$term_id], 'khu_vuc');
            }
        }

        $log[] = "✅ Dòng {$row_num}: [{$action_label}] {$ten_quan} (ID: {$post_id})";
    }
